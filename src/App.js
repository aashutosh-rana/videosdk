...
import { GetToken, callMeeting } from "./videoapi";
...
const App = () => {
  const headers = {
    ...
    appVersionCode: 1,
    role: "EXPERT",
    Authorization: "4e015324-439c-42ca-bc49-8234894a647b"
  };

  const [token, setToken] = useState("");
  const [meetingId, setMeetingId] = useState("ydqh-0y5y-2tn5");
  ...
  useEffect(() => {
    axios
      .get(`https://api.mindzenx.com/video-sdk/auth-token`, { headers })
      .then((response) => {
